...
export class GridOverlay {
    ...
    async attach(): Promise<void> {
        ...
        await this.page.evaluate(([c, id, grid]) => {
            ...
            function align() {
                ...
                const p = (host as HTMLElement).getBoundingClientRect();
            ...
            const ro = new ResizeObserver(align); ro.observe(host); ro.observe(can);
            const mo = new MutationObserver(align); mo.observe(host, { attributes: true, subtree: true });
            (overlay as any).__cleanup = () => { ro.disconnect(); mo.disconnect(); };
    ...
    async setInteractive(interactive: boolean): Promise<void> {
        await this.page.evaluate(([id, interactive]) => {
            const overlayId = id as string;
            const isInteractive = interactive as boolean;
            const el = document.getElementById(overlayId);
            if (el) el.style.pointerEvents = isInteractive ? 'auto' : 'none';
        }, [this.overlayId, interactive]);
    }

    async toggleVisibility(show: boolean): Promise<void> {
        await this.page.evaluate(([id, show]) => {
            const overlayId = id as string;
            const showOverlay = show as boolean;
            const el = document.getElementById(overlayId);
            if (!el) return;
    ...
    async highlightCell(col: number, row: number, options?: {
        ...
        await this.page.evaluate(([rect, opts]) => {
            const cellRect = rect as { x: number; y: number; width: number; height: number };
            const options = opts as { color: string; duration: number; thickness: number };
