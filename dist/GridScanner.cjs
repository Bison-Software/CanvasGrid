...
exports.GridScanner = void 0;
class GridScanner {
    constructor(page, grid, mouseInteractions, tooltipDetector, colorSampler) {
        this.page = page;
        this.grid = grid;
    ...
    async scanAllCellsForTooltips(options) {
        const { hoverDelay = 200, detectionSensitivity = 10, skipEmptyCells = true, progressCallback } = options || {};
        const results = [];
    ...
    async hasAnyTooltips(options) {
        const { maxCellsToCheck = Math.min(20, this.grid.cols * this.grid.rows), hoverDelay = 150, detectionSensitivity = 10 } = options || {};
        ...
        for (let row = 0; row < this.grid.rows; row += stepRow) {
            for (let col = 0; col < this.grid.cols; col += stepCol) {
                ...
                if (cellsToCheck.length >= maxCellsToCheck)
                    break;
            }
            if (cellsToCheck.length >= maxCellsToCheck)
                break;
    ...
    async selectCellsAndExtractText(options) {
        const { startCol, startRow, endCol, endRow, hoverDelay = 300, tooltipSelectors = [
            '[role="tooltip"]',
            '.tooltip',
            '.hover-info',
            '.chart-tooltip',
            ...
            '.tippy-content',
            '.d3-tip'
        ], includeCoordinates = false } = options;
        // Validate bounds
        ...
        for (let row = actualStartRow; row <= actualEndRow; row++) {
            for (let col = actualStartCol; col <= actualEndCol; col++) {
                ...
                try {
                    ...
                    const tooltipInfo = await this.tooltipDetector.hoverCellAndCaptureTooltip(this.mouseInteractions.hoverCell.bind(this.mouseInteractions), col, row, {
                        waitTime: hoverDelay,
                        tooltipSelectors,
...
exports.GridScanner = GridScanner;
