...
const DEFAULT_GRID: Grid = { cols: 8, rows: 6 };

export class CanvasGrid {
    private page: Page;
    private _selector: string | null = null;
    private _locator: Locator | null = null;
    private _nth = 0;
    private _grid: Grid = DEFAULT_GRID;
    private _overlayId = 'pw-canvas-grid';
    ...
    gridSize(cols?: number, rows?: number) {
        if (cols && rows) this._grid = { cols, rows };
        return this;
    }

    /** Heuristic "auto grid": coarse for small canvases, denser for big ones */
    private async autoGridIfNeeded() {
        if (this._grid !== DEFAULT_GRID) return;
        const canvas = await this.canvas();
        const rect = await canvas.boundingBox();
        if (!rect) return;
        const maxDim = Math.max(rect.width, rect.height);
        // simple buckets
        if (maxDim < 400) this._grid = { cols: 6, rows: 4 };
        else if (maxDim < 900) this._grid = { cols: 10, rows: 8 };
        else this._grid = { cols: 14, rows: 10 };
    }
    ...
    async hoverCellAndCaptureTooltip(col: number, row: number, options?: {
        waitTime?: number;
        tooltipSelectors?: string[];
        captureScreenshot?: boolean;
    }) {
        return await this.tooltipDetector.hoverCellAndCaptureTooltip(
     ...
     */
    async hoverCellAndDetectCanvasTooltip(col: number, row: number, options?: {
        waitTime?: number;
        sampleRegion?: { width: number; height: number };
        compareWithBase?: boolean;
    }) {
        return await this.tooltipDetector.hoverCellAndDetectCanvasTooltip(
            this.mouseInteractions.hoverCell.bind(this.mouseInteractions),
            col, row, options
        );
    }
    ...
    async getVisibleTooltips(tooltipSelectors?: string[]) {
     ...
     */
    async highlightCell(col: number, row: number, options?: {
        color?: string;
        duration?: number;
        thickness?: number;
    }) {
    ...
    /** internal */
    private async canvas(): Promise<Locator> {
        if (!this._locator) throw new Error('call .locator(selector) first');
        const loc = this._locator.nth(this._nth);
        if (!(await loc.count())) throw new Error(`no elements match "${this._selector}" at index ${this._nth}`);
        return loc;
    }
